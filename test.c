...
{
    stack_element *el = malloc(sizeof(stack_element));
    el->next = astack->top;
    el->value = value;
    astack->top = el;
}
...
void process(stack* astack, char* token)
{
    ...
    if (is_number(token)) {
        ...
        float val1 = stack_pop(astack);
        float val2 = stack_pop(astack);
        if (is_add(token)) {
            stack_push(astack, val1 + val2);
        } else if (is_sub(token)) {
            stack_push(astack, val2 - val1);
